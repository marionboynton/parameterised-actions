...
class ActorDiscrete(nn.Module):
    ...
    def evaluate(self, state, action):
         ...
         #print("discrete action_logprobs")
...
class ActorContinuous(nn.Module):
    ...
    def get_action(self, state, deterministic=False, explore=False):
         ...
         if explore:
            ...
            #variance = torch.full((self.action_dim,), 1).to(device)
         else:
            action = distribution.sample()
         #distribution = Normal(action_avg, action_std)
...
class HPPO(nn.Module):
    ...
    def __init__(self,
        ...
        self.discrete_policy = ActorDiscrete(self.state_size, self.num_discrete_actions).to(self.device)
        self.old_discrete_policy = ActorDiscrete(self.state_size, self.num_discrete_actions).to(self.device)
        ...
        self.cont_policy = ActorContinuous(self.state_size, self.cont_action_dim, self.action_std).to(self.device)
        self.old_cont_policy = ActorContinuous(self.state_size, self.num_discrete_actions, self.action_std).to(self.device)
    ...
    def update_network(self):
        ...
        for _ in range(self.num_epochs):
            ...
            surr1 = discrete_ratios * advantages
            surr2 = torch.clamp(discrete_ratios, 1-self.eps_clip, 1+self.eps_clip) * advantages
            # final loss of clipped objective PPO
            discrete_loss = -torch.min(surr1, surr2) + 0.5*(self.loss_func(state_values, returns)) - self.ent_coef_discrete*discrete_dist_entropy
            ...
            surr1 = cont_ratios * advantages
            surr2 = torch.clamp(cont_ratios, 1-self.eps_clip, 1+self.eps_clip) * advantages
            cont_loss = -torch.min(surr1, surr2) + self.vf_coef*(self.loss_func(state_values, returns)) - self.ent_coef_cont*cont_dist_entropy
        ...
        self.old_discrete_policy.load_state_dict(self.discrete_policy.state_dict())
        self.old_cont_policy.load_state_dict(self.cont_policy.state_dict())
        #update_target_network(self.critic, self.target_critic, tau=0.001, soft=False)

        # clear buffer
        self.buffer.clear()

        #print(type(discrete_losses), type(cont_losses), type(advantages_avg))
